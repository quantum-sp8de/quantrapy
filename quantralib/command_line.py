...
from .keys import EOSKey, Signer
...
def parse_keys(private_keys):
    if not isinstance(private_keys, list):
        if not isinstance(private_keys, Signer):
            return [EOSKey(private_keys)]
    
    new_priv_keys = []
    for priv_key in private_keys:
        if not isinstance(priv_key, Signer):
                new_priv_keys.append(EOSKey(priv_key))
    return new_priv_keys
...
def cleos():
    ...
    # PUSH
        ...
        if args.push == 'action':
            priv_keys = parse_keys(parse_key_file(args.key_file))
            ...
            print(payload)
            trx = {"actions": [payload]}
            resp = ce.push_transaction(trx, priv_keys, broadcast=args.broadcast)
            console_print(resp)
    # MULISIG
